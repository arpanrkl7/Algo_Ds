...
class LinkedList{
    ...
    int midElement(){
        Node* node=midNode(this->head,this->tail);
        return node->data;
    }
    ...
    int add(Node* ohead,Node* thead,int osize,int tsize){
        if(osize == 0 && tsize==0){
            return 0;
        ...
        if(osize>tsize){
            temp1=ohead->data;
            temp2=0;
            carry=add(ohead->next,thead,osize--,tsize);
        }
        else if(tsize>osize){
            temp1=0;
            temp2=thead->data;
            carry=add(ohead,thead->next,osize,tsize--);
        }
        else{
            temp1=ohead->data;
            temp2=thead->data;
            carry=add(ohead->next,thead->next,osize--,tsize--);
        }
    ...
    void add(LinkedList& ll1,LinkedList& ll2){
        int carry=add(ll1.head,ll2.head,5,6);
        if(carry>0){
            this->addFirst(carry);
        }
    ...
    void mergeTwoSortedList(LinkedList& ll1,LinkedList& ll2){
        ...
        if(first->data>=second->data){
            this->addLast(second->data);
            second=second->next;

        }

        else if(first->data < second->data){
            this->addLast(first->data);
            first=first->next;
        }
        }
    ...
    LinkedList* mergeSort(Node* sp,Node* ep){
        ...
        LinkedList* ll11=mergeSort(sp,mid);
        LinkedList* ll22=mergeSort(mid->next,ep);
        LinkedList* mres=new LinkedList();
        mres->mergeTwoSortedList(*ll11,*ll22);
        return mres;
