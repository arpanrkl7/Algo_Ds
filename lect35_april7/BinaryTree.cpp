...
//      mp.lHeight = height(root->left);
//      mp.rHeight = height(root->right);
//      mp.isBalanced = lHeight-rHeight >=-1 && lHeight-rHeight<=1 ? true : false;
...
//     balanced* myres = new balanced();
//     myres->isbalance = left->isbalance && right->isbalance && (left->h - right->h == -1 || left->h - right->h == 1 || left->h = right->h );
//     myres->h = max(left->h,right->h)+1;
...
//     bst* l = isBst(root->left);
//     bst* r = isBst(root->right);

//     bst* myres = new bst();
//     myres->isBst = l->isBst && r->isBst && root->data > l->max && root->data < r->min;
//     myres->max = max(root->data,max(l->max,r->max));
//     myres->min = min(root->data,min(l->min,r->min));
//     return myres;
// }
...
bst* findlBst(Node* root){
    if(root == NULL){
        bst* bres = new bst();
        bres->isBst = true;
        bres->max = INT_MIN;
        bres->min = INT_MAX;
        bres->lbstr = 0;//largest bst root
        bres->lbsts = 0;//largest bst size
        return bres;
    }

    bst* l = findlBst(root->left);
    bst* r = findlBst(root->right);
...
//    vector<int> pre{10,20,40,30,50};
//    vector<int> in{40,20,10,30,50};
//    Node* root5 = construct1(pre,in,0,4,0,4);
// //    Node* res= isBalanced(root5);
...
//    cout << res->isBst << endl;
   ...
   Node* root = construct(dlist);
   bst* rses = findlBst(root);
   cout << res->lbsts << " " << res->lbstr->data << endl;
